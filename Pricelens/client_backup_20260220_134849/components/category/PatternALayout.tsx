...
export default function PatternALayout({
  ...
  const displayProducts = useMemo(() => {
    ...
    return Array.isArray(products) ? products.filter(p => {
      ...
      // Accept any http(s) image so backend results show (including placeholder when Serper has no image)
      const hasValidImage = p.image && 
        typeof p.image === 'string' && 
        p.image.trim().length > 0 &&
        (p.image.startsWith('http://') || p.image.startsWith('https://')) &&
        !p.image.includes('example.com'); // Exclude example.com test URLs only
  ...
  }, [hasSearched, searchQuery, searchResults, defaultProductsList, selectedSubcategory, subcategoryProducts]);
          ...
          <View style={{
            ...
            <View style={{ width: '100%', position: 'relative' }}>
              <View style={{
                flexDirection: 'row',
                alignItems: 'center',
                backgroundColor: (categorySlug === 'groceries' || categorySlug === 'electronics')
                  ? 'rgba(255, 255, 255, 0.08)' 
                  : 'rgba(255, 255, 255, 0.05)',
                borderRadius: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 12 : 8,
                borderWidth: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 1.5 : 1,
                borderColor: (categorySlug === 'groceries' || categorySlug === 'electronics')
                  ? 'rgba(96, 165, 250, 0.3)' 
                  : 'rgba(139, 149, 168, 0.2)',
                paddingHorizontal: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 16 : 12,
                height: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 52 : 40,
                shadowColor: (categorySlug === 'groceries' || categorySlug === 'electronics') ? '#60a5fa' : 'transparent',
                shadowOffset: (categorySlug === 'groceries' || categorySlug === 'electronics') ? { width: 0, height: 2 } : { width: 0, height: 0 },
                shadowOpacity: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 0.2 : 0,
                shadowRadius: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 4 : 0,
                elevation: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 2 : 0,
              }}>
                <Ionicons 
                  name="search" 
                  size={(categorySlug === 'groceries' || categorySlug === 'electronics') ? 22 : 18} 
                  color={(categorySlug === 'groceries' || categorySlug === 'electronics') ? '#60a5fa' : '#8b95a8'} 
                  style={{ marginRight: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 12 : 8 }} 
                />
                <TextInput
                  placeholder={
                    categorySlug === 'groceries'
                      ? "Search groceries... (e.g., Apple, Milk, Bread)"
                  ...
                  placeholderTextColor={(categorySlug === 'groceries' || categorySlug === 'electronics')
                    ? "rgba(139, 149, 168, 0.7)" 
                    : "rgba(139, 149, 168, 0.6)"}
                  value={searchQuery}
                  onChangeText={(text) => {
                  ...
                  style={{
                    flex: 1,
                    color: '#e8edf4',
                    fontSize: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 16 : 14,
                    lineHeight: (categorySlug === 'groceries' || categorySlug === 'electronics') ? 24 : 20,
                    paddingVertical: 0,
                    includeFontPadding: false,
                    textAlignVertical: 'center',
                    fontWeight: (categorySlug === 'groceries' || categorySlug === 'electronics') ? '500' : '400',
                  }}
                />
                {isSearching && (
