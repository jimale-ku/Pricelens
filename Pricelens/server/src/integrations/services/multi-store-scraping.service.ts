...
export class MultiStoreScrapingService {
  ...
  private isValidPriceForProduct(price: number, productName: string): boolean {
    ...
    // Global cap: no single item should be over $50,000 (catches API/parsing errors)
    if (price > 50000) {
      this.logger.warn(`   ⏭️ REJECTING price: $${price} (global max $50,000) - product: "${productName}"`);
      return false;
    }

    // Groceries / food: fruit, milk, vegetables, etc. should never be over $200 per item
    const isGroceryOrFood =
      /\b(fruit|fruits|apple|banana|bananas|orange|oranges|grape|grapes|berry|berries|milk|organic milk|cream|yogurt|egg|eggs|bread|vegetable|vegetables|tomato|lettuce|onion|potato|carrot|broccoli|spinach|avocado|strawberry|blueberry|blackberry|raspberry|watermelon|melon|cantaloupe|peach|pear|plum|cherry|mango|pineapple|coconut|kiwi|lime|lemon|lemons)\b/i.test(productNameLower) ||
      productNameLower.includes('organic') && (productNameLower.includes('produce') || productNameLower.includes('fruit') || productNameLower.includes('vegetable') || productNameLower.includes('grocery')) ||
      /\b(grocery|groceries|produce|dairy|bakery)\b/i.test(productNameLower);
    if (isGroceryOrFood && price > 200) {
      this.logger.warn(`   ⏭️ REJECTING grocery/food price: $${price} (max $200) - product: "${productName}"`);
      console.warn(`   ⚠️ [Price Validation] REJECTING unrealistic price $${price} for food/grocery: "${productName}"`);
      return false;
    }
  ...
  async getAllStorePricesFromSerpAPI(
    ...
    try {
      ...
      const isElectronics = (productNameLower.includes('iphone') || 
                            ...
                            productNameLower.includes('lenovo') ||
                            productNameLower.includes('laptop') ||
