...
import { getStoreLogoSource, type StoreLogoSource } from '@/constants/storeAssets';
...
export function transformProduct(backendProduct: BackendProduct): FrontendProduct | null {
  ...
  // Handle prices: backend may send .prices (Prisma) or .storePrices (enriched/compare response)
  const rawPrices = (backendProduct as any).prices ?? (backendProduct as any).storePrices ?? [];
  ...
  const normalizedPrices = prices
    .map((p: any) => {
      const n = toNumberOrNull(p?.price ?? p?.totalPrice);
      return { ...p, __priceNumber: n, store: p.store || (p.storeName && { name: p.storeName, logo: p.storeImage }) };
    })
  ...
  let storePrices: FrontendStorePrice[] = sortedPrices.map((price, index) => {
    ...
    const storeImage = getStoreLogoSource(storeName, price.store?.logo);
