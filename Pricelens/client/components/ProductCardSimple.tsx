...
export default function ProductCardSimple({
  ...
  const handleAddToList = async () => {
    ...
    console.log('➕ Adding to list:', { productId, productName, category });
    setIsAddingToList(true);
    try {
      // Get current list (grocery or default)
      const currentList = await getCurrentList();
      console.log('📋 Current list:', { id: currentList.id, name: currentList.name });
      ...
      if (result.success) {
        ...
        console.error('❌ Failed to add to list:', result.message);
        Alert.alert('❌ Error', result.message || 'Failed to add item to list');
      }
    } catch (error) {
      console.error('❌ Error adding to list:', error);
      Alert.alert('❌ Error', `Failed to add item to list: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  // Load images immediately - removed delay to ensure images always show
  useEffect(() => {
    setShouldLoadImage(true);
  }, []);

  // Determine final image URL - always use placeholder if no valid image
  ...
  if (productImage && typeof productImage === 'string') {
    ...
    // Accept any valid URL (including placeholders) - ensure images always show
    if (isValidUrl && !isTestUrl && trimmed.length >= 10) {
      finalImageUri = trimmed;
    } else {
      // If invalid, use category placeholder
        ...
        <TouchableOpacity
          ...
          style={{
            ...
            backgroundColor: addedToList
              ? 'rgba(16, 185, 129, 0.2)' // Green when added
              : isAddingToList 
              ? 'rgba(59, 130, 246, 0.2)' 
              : 'rgba(59, 130, 246, 0.15)',
            borderWidth: 1,
            borderColor: addedToList
              ? 'rgba(16, 185, 129, 0.4)' // Green border when added
              : isAddingToList 
              ? 'rgba(59, 130, 246, 0.4)' 
              : 'rgba(59, 130, 246, 0.3)',
            opacity: (isAddingToList || addedToList) ? 0.7 : 1,
          }}
        >
          <Ionicons 
            name={addedToList ? "checkmark" : isAddingToList ? "hourglass-outline" : "add"} 
            size={18} 
            color={addedToList ? "#10b981" : "#3B82F6"} 
          />
        </TouchableOpacity>
      </View>
    </View>
