...
import type { StoreLogoSource } from '@/constants/storeAssets';
import { getStoreLogoUrl } from '@/constants/storeLogos';

interface StoreCardProps {
  rank: number;
  storeName: string;
  price: string;
  /** Logo: local asset (number) or URL string */
  storeImage: StoreLogoSource;
  isBestDeal?: boolean;
...
export default function StoreCard({ 
  ...
  const isLocalAsset = typeof storeImage === 'number';
  const showLetter = logoError || (storeImage == null || storeImage === '');
  const initial = (storeName || '?').replace(/^[^a-zA-Z0-9]+/, '').charAt(0).toUpperCase() || '?';

  const renderStoreLogo = (containerStyle: object) => {
    if (showLetter) {
      return (
        <View style={[containerStyle, { width: 40, height: 40, borderRadius: 20, backgroundColor: '#334155', alignItems: 'center', justifyContent: 'center' }]}>
          <Text style={{ fontSize: 18, fontWeight: '700', color: '#e2e8f0' }}>{initial}</Text>
        </View>
      );
    }
    return (
      <Image
        source={isLocalAsset ? storeImage : { uri: storeImage as string }}
      ...
      if (canOpen) {
        ...
        if (productId && productName && productImage && category) {
          try {
            ...
            const storeImageForRecord = typeof storeImage === 'number' ? getStoreLogoUrl(storeName, null) : storeImage;
