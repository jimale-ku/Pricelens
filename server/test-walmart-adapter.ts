/**
 * Test Walmart Adapter
 * 
 * Tests the Walmart Open API adapter
 * 
 * Usage:
...
import { WalmartAdapter } from './src/integrations/adapters/walmart/walmart.adapter';
...
async function testWalmartAdapter() {
  console.log('🧪 Testing Walmart Adapter\n');

  const configService = new MockConfigService();
  const adapter = new WalmartAdapter(configService as any);
  ...
  if (!isEnabled) {
    console.log('\n⚠️  Walmart adapter is not configured!');
    console.log('💡 Add to server/.env:');
    console.log('   WALMART_API_KEY=your_key');
    console.log('\n📖 Get API key from: https://developer.walmart.com/');
    return;
  }
  ...
  console.log('\n✅ Walmart adapter test complete!');
}

// Run tests
testWalmartAdapter().catch((error) => {
